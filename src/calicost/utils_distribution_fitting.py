...
class Weighted_NegativeBinomial_mix(GenericLikelihoodModel):
    ...
    def nloglikeobs(self, params):
        mean = self.exposure * (
            self.tumor_prop * (self.exog @ np.exp(params[:-1])) + 1.0 - self.tumor_prop
        )
        var = mean + params[-1] * mean**2.0
    ...
    @profile
    def fit(
        self, start_params=None, maxiter=10_000, maxfun=5_000, verbose=False, **kwargs
    ):
        self.exog_names.append("alpha")
...
class Weighted_BetaBinom(GenericLikelihoodModel):
    ...
    def nloglikeobs(self, params):
        ...
        b = self.exog @ (1.0 - params[:-1]) * params[-1]
        ...
        if start_params is None:
            if hasattr(self, "start_params"):
                ...
                start_params = np.append(
                    0.5 / np.sum(self.exog.shape[1]) * np.ones(self.nparams), start_tau
                )
...
class Weighted_BetaBinom_mix(GenericLikelihoodModel):
    ...
    def nloglikeobs(self, params):
        ...
        a = (
            self.exog @ params[:-1] * self.tumor_prop + 0.5 * (1.0 - self.tumor_prop)
        ) * params[-1]
        b = (
            self.exog @ (1.0 - params[:-1]) * self.tumor_prop
            + 0.5 * (1.0 - self.tumor_prop)
        ) * params[-1]
...
class Weighted_BetaBinom_fixdispersion(GenericLikelihoodModel):
    ...
    def fit(
        self, start_params=None, maxiter=10000, maxfun=5000, verbose=False, **kwargs
    ):
        if start_params is None:
            if hasattr(self, "start_params"):
                start_params = self.start_params
...
class Weighted_BetaBinom_fixdispersion_mix(GenericLikelihoodModel):
    def __init__(self, endog, exog, tau, weights, exposure, tumor_prop, **kwargs):
        super(Weighted_BetaBinom_fixdispersion_mix, self).__init__(
            endog, exog, **kwargs
        )
        self.tau = tau
        self.weights = weights
