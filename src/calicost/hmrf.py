...
def hmrf_reassignment_posterior(
    ...
    for i in trange(N, desc="hmrf_reassignment_posterior"):
        ...
        single_base_idx_sum1 = np.sum(single_base_nb_mean[:,idx], axis=1, keepdims=True)
        single_total_idx_sum1 = np.sum(single_total_bb_RD[:,idx], axis=1, keepdims=True)
        
        for c in range(n_clones):
            tmp_log_emission_rdr, tmp_log_emission_baf = hmmclass.compute_emission_probability_nb_betabinom(
                single_X_idx_sum2,
                single_base_idx_sum1, 
                res["new_log_mu"][:,c:(c+1)],
                res["new_alphas"][:,c:(c+1)],
                single_total_idx_sum1, 
...
def hmrf_pipeline(outdir, single_X, lengths, single_base_nb_mean, single_total_bb_RD, initial_clone_index, n_states, \
    ...
    logger.info("Running HMRF pipeline.")
        
    # HMM
    for r in trange(max_iter_outer, desc="hmrf_pipeline"):
        if not Path(f"{outdir}/round{r}_nstates{n_states}_{params}.npz").exists():
            ...
            logger.info("Assigning clones (step {r}).")
            ...
            logger.info("Assigned clones (step {r}).")
            ...
            logger.info(f"Writing to {outdir}/round{r}_nstates{n_states}_{params}.npz")
...
def hmrf_concatenate_pipeline(outdir, prefix, single_X, lengths, single_base_nb_mean, single_total_bb_RD, initial_clone_index, n_states, \
    ...
    # HMM
    for r in trange(max_iter_outer, desc="hmrf_concatenate_pipeline"):
        ...
        if allres["num_iterations"] > r:
            ...
            logger.info(f"Writing to {outdir}/{prefix}_nstates{n_states}_{params}.npz")
...
def hmrfmix_reassignment_posterior(
    ...
    for i in trange(N, desc="hmrfmix_reassignment_posterior", mininterval=4.):
        idx = smooth_mat[i,:].nonzero()[1]
        idx = idx[~np.isnan(single_tumor_prop[idx])]
...
def hmrfmix_pipeline(outdir, prefix, single_X, lengths, single_base_nb_mean, single_total_bb_RD, single_tumor_prop, initial_clone_index, n_states, log_sitewise_transmat, \
    ...
    for r in range(max_iter_outer):
        ...
        if allres["num_iterations"] > r:
            ...
            logger.info("Writing to {outdir}/{prefix}_nstates{n_states}_{params}.npz")
    ...
    for i in trange(N, desc="aggr_hmrfmix_reassignment_concatenate"):
        ...
        for c in range(n_clones):
            ...
            tmp_log_emission_rdr, tmp_log_emission_baf = hmmclass.compute_emission_probability_nb_betabinom_mix(
                ...
                res["new_p_binom"],
                res["new_taus"],
                np.ones((n_obs,1)) * np.mean(single_tumor_prop[idx]), weighted_tp.reshape(-1,1)
            )
...
def hmrfmix_concatenate_pipeline(outdir, prefix, single_X, lengths, single_base_nb_mean, single_total_bb_RD, single_tumor_prop, initial_clone_index, n_states, log_sitewise_transmat, \
    ...
    for r in trange(max_iter_outer, desc="HMRF (hmrfmix_concatenate_pipeline)", leave=True):
        ...
        if allres["num_iterations"] > r:
            ...
            logger.info(f"Assigning clones (step {r})")
            ...
            logger.info(f"Clones assigned (step {r})")
        ...
        logger.info("Merging to pseudobulk.")
